...
function dropCachedDB(reason='unknown'){
  const db=cachedDB;cachedDB=null;openingPromise=null;
  try{db?.close()}catch{}
  bb('DB_CONNECTION_RESET',{reason});
}
...
export function openDB(){
  ...
  openingPromise=new Promise((resolve,reject)=>{
    ...
    const finishError=(err)=>{if(settled)return;settled=true;clearTimeout(timer);openingPromise=null;reject(err)};
    ...
    request.onerror=()=>{bb('DB_OPEN_ERROR',{name:request.error?.name,message:request.error?.message});finishError(request.error||new Error('Little Moments storage could not open.'))};
  });
  return openingPromise;
}

export async function put(store,value){const db=await openDB();return new Promise((resolve,reject)=>{let tx;try{tx=db.transaction(store,'readwrite')}catch(e){dropCachedDB('put-transaction-failed');reject(e);return}tx.objectStore(store).put(value);tx.oncomplete=()=>{bb('DB_WRITE',{operation:'put',store,key:value?.id??value?.key??null});emitChange({operation:'put',store,key:value?.id??value?.key??null});resolve(value)};tx.onabort=()=>bb('DB_WRITE_ABORT',{operation:'put',store,error:String(tx.error||'')});tx.onerror=()=>{bb('DB_WRITE_ERROR',{operation:'put',store,error:String(tx.error||'')});reject(tx.error)}})}
export async function remove(store,key){const db=await openDB();return new Promise((resolve,reject)=>{let tx;try{tx=db.transaction(store,'readwrite')}catch(e){dropCachedDB('remove-transaction-failed');reject(e);return}tx.objectStore(store).delete(key);tx.oncomplete=()=>{bb('DB_WRITE',{operation:'remove',store,key});emitChange({operation:'remove',store,key});resolve(key)};tx.onabort=()=>bb('DB_WRITE_ABORT',{operation:'remove',store,key,error:String(tx.error||'')});tx.onerror=()=>{bb('DB_WRITE_ERROR',{operation:'remove',store,key,error:String(tx.error||'')});reject(tx.error)}})}

export async function clearStore(store){const before=await liveCounts().catch(()=>null);audit({operation:'clearStore',store,before});bb('DB_DESTRUCTIVE_ATTEMPT',{operation:'clearStore',store,before});const db=await openDB();return new Promise((resolve,reject)=>{let tx;try{tx=db.transaction(store,'readwrite')}catch(e){dropCachedDB('clear-transaction-failed');reject(e);return}tx.objectStore(store).clear();tx.oncomplete=()=>{bb('DB_DESTRUCTIVE_COMPLETE',{operation:'clearStore',store});emitChange({operation:'clearStore',store});resolve()};tx.onabort=()=>bb('DB_DESTRUCTIVE_ABORT',{operation:'clearStore',store,error:String(tx.error||'')});tx.onerror=()=>{bb('DB_DESTRUCTIVE_ERROR',{operation:'clearStore',store,error:String(tx.error||'')});reject(tx.error)}})}
...
export async function getAll(store){const db=await openDB();return new Promise((resolve,reject)=>{let tx;try{tx=db.transaction(store,'readonly')}catch(e){dropCachedDB('read-transaction-failed');reject(e);return}const req=tx.objectStore(store).getAll();req.onsuccess=()=>resolve(req.result);req.onerror=()=>{bb('DB_READ_ERROR',{store,error:String(req.error||'')});reject(req.error)}})}
export async function get(store,key){const db=await openDB();return new Promise((resolve,reject)=>{let tx;try{tx=db.transaction(store,'readonly')}catch(e){dropCachedDB('get-transaction-failed');reject(e);return}const req=tx.objectStore(store).get(key);req.onsuccess=()=>resolve(req.result);req.onerror=()=>{bb('DB_READ_ERROR',{store,key,error:String(req.error||'')});reject(req.error)}})}
